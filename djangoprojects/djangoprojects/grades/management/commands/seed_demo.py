...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for code, name in courses_data:
            course, c_created = Course.objects.get_or_create(code=code, defaults={'name': name, 'teacher': teacher_obj})
            if c_created:
                created.append(f'course {code}')
            else:
                # ensure teacher set
                if course.teacher != teacher_obj:
        ...
        for course in courses:
            en, e_created = Enrollment.objects.get_or_create(student=student, course=course, defaults={'semester': '2026S', 'midtrem_grade': 85.0, 'final_grade': 90.0})
            if e_created:
